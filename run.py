...
if (
	...
	def get_gpt_command(objective, url, previous_command, browser_content, detected_food):
		prompt = prompt_template
		prompt = prompt.replace("$objective", objective)
		prompt = prompt.replace("$url", url[:100])
		prompt = prompt.replace("$previous_command", previous_command)
		prompt = prompt.replace("$browser_content", browser_content[:4500])
		prompt = prompt.replace("$detected_food", str(detected_food))
	...
	# object detection from yolov5
	# PyTorch Hub Model
	#model = torch.hub.load('ultralytics/yolov5', 'yolov5s')  # or yolov5n - yolov5x6, custom
	best_model = "yolov5_configs/best_v2.pt"
	model = torch.hub.load("yolov5", "custom", path=best_model, source="local")

	# Images
	img = "images/Fridge_FEATURE.webp"
	...
	# Results
	detected_classes = results.xyxy[0][:,5].tolist()
	# # convert class to label
	with open('yolov5_configs/data.yaml') as f:
    
		data = yaml.load(f, Loader=yaml.FullLoader)
		print(data["names"])
		print(len(data["names"]))
	detected_food = [data["names"][int(i)] for i in detected_classes] 
	print("{} detected by yolov5".format(detected_food))



	# natbot begins 
	objective = "What can I cook?"
	...
	try:
		while True:
			browser_content = "\n".join(_crawler.crawl())
			prev_cmd = gpt_cmd
			gpt_cmd = get_gpt_command(objective, _crawler.page.url, prev_cmd, browser_content, detected_food)
